...
if __name__ == '__main__':
    ...
    parser.add_argument('--input2', default='DB2_H', help='HAAR_H, HAAR_L, HAAR_HHL, HAAR_HLL, BIOR1.5_H, BIOR2.4_H, COIF1_H, DB2_H, DMEY_H')
    parser.add_argument('--sup_mark', default='100', help='1, 5, 10, 100')
    parser.add_argument('-b', '--batch_size', default=32, type=int)
    parser.add_argument('-e', '--num_epochs', default=200, type=int)
    parser.add_argument('-s', '--step_size', default=50, type=int)
    parser.add_argument('-l', '--lr', default=0.5, type=float)
    parser.add_argument('-g', '--gamma', default=0.5, type=float)
    parser.add_argument('--cps_weight', default=1, type=float)
    parser.add_argument('--em_weight', default=1, type=float)
    ...
    path_trained_models = path_trained_models + '/' + 'xnet_em' + '-l=' + str(args.lr) + '-e=' + str(args.num_epochs) + '-s=' + str(args.step_size) + '-g=' + str(args.gamma) + '-b=' + str(args.batch_size) + '-cw=' + str(args.cps_weight) + '-em=' + str(args.em_weight) + '-w=' + str(args.warm_up_duration) + '-' + str(args.sup_mark)+'-'+str(args.input1)+'-'+str(args.input2)
    if not os.path.exists(path_trained_models) and rank == args.rank_index:
        os.mkdir(path_trained_models)
    ...
    path_seg_results = path_seg_results + '/' + 'xnet_em' + '-l=' + str(args.lr) + '-e=' + str(args.num_epochs) + '-s=' + str(args.step_size) + '-g=' + str(args.gamma) + '-b=' + str(args.batch_size) + '-cw=' + str(args.cps_weight) + '-em=' + str(args.em_weight) + '-w=' + str(args.warm_up_duration) + '-' + str(args.sup_mark)+'-'+str(args.input1)+'-'+str(args.input2)
    if not os.path.exists(path_seg_results) and rank == args.rank_index:
        os.mkdir(path_seg_results)

    if args.vis and rank == args.rank_index:
        visdom_env = str('Sup-XNet_EM-' + str(os.path.split(args.path_dataset)[1]) + '-' + args.network + '-l=' + str(args.lr) + '-e=' + str(args.num_epochs) + '-s=' + str(args.step_size) + '-g=' + str(args.gamma) + '-b=' + str(args.batch_size)+ '-cw=' + str(args.cps_weight) + '-em=' + str(args.em_weight) + '-w=' + str(args.warm_up_duration) + '-' + str(args.sup_mark)+'-'+str(args.input1)+'-'+str(args.input2))
    ...
    for epoch in range(args.num_epochs):
        ...
        cps_weight = args.cps_weight * (epoch + 1) / args.num_epochs
        em_weight = args.em_weight * (epoch + 1) / args.num_epochs
        ...
        for i, data in enumerate(dataloaders['train']):
            ...
            outputs_train1_soft = torch.softmax(outputs_train1, 1)
            outputs_train2_soft = torch.softmax(outputs_train2, 1)
            ...
            loss_train_cps = criterion(outputs_train1, max_train2) + criterion(outputs_train2, max_train1)
            loss_train_cps = loss_train_cps * cps_weight
            loss_train_em = em_weight * (entropy_loss(outputs_train2_soft, C=2) + entropy_loss(outputs_train1_soft, C=2))
            loss_train_unsup = loss_train_cps + loss_train_em
